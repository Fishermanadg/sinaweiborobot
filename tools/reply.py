...
def reply(client=None):
    if client == None:
        ...
        last_callme_comment = client.comments.to_me.get(
            filter_by_author=1)['comments'][0]
        last_callme_comment_id = last_callme_comment['id']
        # print 'last_callme_comment_id:',last_callme_comment_id
        last_callme_comment_username = last_callme_comment[
            'user']['name'].encode('utf-8')
        last_callme_comment_created_at = last_callme_comment[
            'created_at'].encode('utf-8')
        last_callme_comment_content = last_callme_comment[
            'text'].encode('utf-8')
        last_comment_weibo_id = last_callme_comment['status']['id']
